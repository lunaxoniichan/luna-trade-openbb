...
Fork note: null-guard catalog fields in ``build_choices``. Upstream CFTC
catalog drift can return ``None`` for subcategory/code/name; stripping those
crashed FastAPI startup and blocked the whole platform API (port 6900).
"""
...
async def build_choices():
    ...
    for d in contracts:
        ...
        # Skip entries missing required label/value fields.
        if not name or not code:
            continue
        label_tail = f"{subcategory}  | {code}" if subcategory else code
